...
The **K-Event-Processor** is a KSP plugin that detects all `@Register` methods at compile time and automatically generates an `object` as well as a file under `META-INF/services`. This allows the EventManager to **find handlers faster at runtime**.  

### Usage

To use the K-Event-Processor, you need to add a new component when configuring the `EventManager`:

```kotlin
val processorRegistry = ListenerProcessorRegistry.create()
...
Here, `ListenerProcessorRegistry.create()` creates the necessary registry, which the EventManager uses to efficiently register and dispatch handlers.

### Fallback

If the `DefaultEventManager` does **not** find a registry generated by K-Event-Processor, it will fall back to the standard **reflection-based handler discovery**.

> Benefit: Using the K-Event-Processor reduces runtime reflection and improves event dispatch performance while maintaining full backwards compatibility.
...
dependencies {
    ...
    ksp("com.fantamomo:k-event-processor:${processor-version}")
}
...
API-Version: **1.8-SNAPSHOT**  
Manager-Version: **1.19-SNAPSHOT**  
Processor-Version: **1.0-SNAPSHOT**
